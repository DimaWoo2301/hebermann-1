...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		$this->document->addStyle('catalog/view/theme/default/stylesheet/catalog_product.css');
		$this->document->addScript('catalog/view/theme/default/js/product_swiper.js', 'footer');
		...
		if ($category_info) {
			...
			if ($category_info['image2']) {
				$data['thumb_image2'] = $this->model_tool_image->resize($category_info['image2'], 1420, 470); 
			} else {
				$data['thumb_image2'] = '';
			}			
			...
			foreach ($results as $result) {
				...
				$data['categories'][] = array(
					...
					'thumb' => $result['image'] 
						? $this->model_tool_image->resize($result['image'], 650, 298) 
						: $this->model_tool_image->resize('no_image.png', 650, 298),
			...
			foreach ($results as $result) {
				...
				if ($additional_images) {
					$images = array_map(function ($image) {
						return $this->model_tool_image->resize($image['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_height'));
					}, $additional_images);
				} else {
					$images = [];
				}
